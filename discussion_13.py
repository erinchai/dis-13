...
def create_employee_table(cur, conn):
    cur.execute('CREATE TABLE employees1 (id INTEGER PRIMARY KEY, first_name TEXT, last_name TEXT, hire_date TEXT, job TEXT, salary INTEGER)')
...
def add_employee(filename, cur, conn):
    ...
    info = (file_data['id'], file_data['first_name'], file_data['last_name'], file_data['hire_date'], file_data['job'], file_data['salary'])
    cur.execute("INSERT INTO employees (id, first_name, last_name, hire_date, job, salary) VALUES (?, ?, ?, ?, ?, ?)",info)
    conn.commit()
